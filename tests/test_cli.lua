...
local invocationErrors = os.tmpname()
local function expectInvocationError(arguments)
    assert(not succeeded("lua src/cli.lua " .. arguments .. " 2>" .. invocationErrors), "invalid invocation must fail")
    local errorFile = assert(io.open(invocationErrors, "r"))
    local message = errorFile:read("*all"); errorFile:close()
    assert(message:find("usage:", 1, true), "invalid invocation must report usage")
end
expectInvocationError("--unknown " .. input)
expectInvocationError("--check " .. input .. " " .. output)
expectInvocationError(input)
expectInvocationError(input .. " " .. output .. " extra.json")
